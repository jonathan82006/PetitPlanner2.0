...
namespace PetitPlannerIntegrador.Repositories
{
    public class UserRepositorie
    {
        ...
        public userModel UserInformation()
        {
            ...
            string query = "SELECT user AS Nombre, email AS Email, id_user AS IdUser FROM usuarios WHERE id_user = @id";
                        ...
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                usuario = new userModel
                                {
                                    ...
                                    Nombre = reader["Nombre"].ToString()!, // <-- Debe coincidir con el alias 'Nombre' del SELECT
                                    Email = reader["Email"].ToString()!,   // <-- Debe coincidir con el alias 'Email' del SELECT
                                };
                            }
                        }
                    }
